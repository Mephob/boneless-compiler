...
public class SemantickiAnalizator {
	...
	//GOTOVO
	public void begin(ASTNode root) {
	...
	private void definicijaFunkcije(ASTNode node) {
		...
		if (size == 6 && !treci.value.equals("<lista_parametara>")) { //prva produkcija
			...
			if (greska) {
				...
				terminal(op2);
				ASTNode op3 = node.children.get(3);
				terminal(op3);
				ASTNode op4 = node.children.get(4);
				terminal(op4);
		...
		if (greska) {
			//todo output
			System.out.print("GRESKA BOKTE MAZO");
			exit(1);
		}
	}
	...
	private void deklaracijaParametra(ASTNode node) {
		int size = node.children.size();
		ASTNode prvi = node.children.get(0);
		if (!prvi.value.equals("<ime_tipa>")) Util.greska("deklaracijaParametra");
	...
	private void bigGreska(ASTNode node) {
		...
		terminal(op1);
		ASTNode op2 = node.children.get(1);
		terminal(op2);
		ASTNode op3 = node.children.get(2);
		terminal(op3);
		ASTNode op4 = node.children.get(3);
		terminal(op4);
...
=======
	...
	private void naredbaPetlje(ASTNode node) {
		...
		if (size == 5) {
			...
			if (!isImplicitlyCastable(node.children.get(2).tip, "KR_INT")) {
				...
				ASTNode dva = node.children.get(1);
				terminal(dva);
				ASTNode tri = node.children.get(3);
				terminal(tri);
	...
	/* ========== MISCELLANEOUS ========== */
>>>>>>> 886aed6d9185985d404e68d16ec3e06302e18e21
